...
class FriendSystem {
  static Future addFriend(UserData currentUserData, String uid) async {
    ...
    UserData otherUserData = UserData(
        ...
        level: 1,
        experiences: 1,
        friendIDs: List.empty(),
        friendIDRequests: List.empty(),
        friendNames: List.empty(),
        friendNameRequests: List.empty(),
        friendLevels: List.empty(),
        friendLevelRequests: List.empty());
    String otherUserId = "";
    ...
    await DatabaseService(uid: currentUserData.uid).updateUserDataFriend(
        currentUserFriendIDList,
        currentUserFriendNameList,
        currentUserFriendLevelList);
    ...
    return await DatabaseService(uid: uid).updateUserDataFriendRequest(
        friendRequestsID, friendRequestsName, friendRequestsLevel);
  }

  static Future acceptFriend(
      UserData currentUserData, String uid, String name, int Level) async {
    var currentUserFriendIDList = currentUserData.friendIDs;
    var currentUserFriendNameList = currentUserData.friendNames;
    var currentUserFriendLevelList = currentUserData.friendLevels;
    ...
    return await DatabaseService(uid: currentUserData.uid).updateUserDataFriend(
        currentUserFriendIDList,
        currentUserFriendNameList,
        currentUserFriendLevelList);
  }

  static Future DeleteFriendRequest(
      UserData currentUserData, String uid, String name, int Level) async {
    ...
    await DatabaseService(uid: currentUserData.uid).updateUserDataFriendRequest(
        currentUserFriendIDList,
        currentUserFriendNameList,
        currentUserFriendLevelList);
  }
